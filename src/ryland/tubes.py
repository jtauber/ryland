...
def project(keys: list[str]) -> Tube:
    def inner(_ryland: "Ryland", context: Context) -> Context:
...
def markdown(frontmatter: bool = False) -> Tube:
    def inner(ryland: "Ryland", context: Context) -> Context:
        ...
        if frontmatter:
            ...
            source_content = context["source_content"]
            extra = {}
        markdown_engine = ryland._markdown  # pyright: ignore[reportPrivateUsage]
        html_content = markdown_engine.convert(source_content)
